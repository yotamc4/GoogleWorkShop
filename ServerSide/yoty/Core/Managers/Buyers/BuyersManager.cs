...
namespace YOTY.Service.Core.Managers.Buyers
{
    public class BuyersManager : IBuyersManager
    {
        ...
        public async Task<Response<BidsDTO>> GetBidsCreatedByBuyer(string buyerId)
        {
            var bids = _context.Bids.Where(b => b.OwnerId == buyerId).Select(bid => _mapper.Map<BidDTO>(bid)).ToList();
            return new Response<BidsDTO>() { DTOObject = BidsDTO.CreateDefaultBidsPage(bids) , IsOperationSucceeded = true, SuccessOrFailureMessage = this.getSuccessMessage() };
        }
        ...
        public async Task<Response<BidsDTO>> GetBuyerBids(string buyerId)
        {
            ...
            List<BidDTO> liveBids = buyer.CurrentParticipancies.Select(p => p.Bid).Select(bid => _mapper.Map<BidDTO>(bid)).ToList();
        ...
        public async Task<Response<BidsDTO>> GetBuyerLiveBids(string buyerId)
        {
        ...
        public async Task<Response<BidsDTO>> GetBuyerOldBids(string buyerId)
        {
            DateTime current_time = DateTime.Now;
            var buyer = await _context.Buyers.Where(b => b.Id == buyerId).Include(b => b.CurrentParticipancies).ThenInclude(p=>p.Bid).FirstOrDefaultAsync().ConfigureAwait(false);
            if (buyer == null)
            ...
            List<BidDTO> oldBids = buyer.CurrentParticipancies.Select(p => p.Bid).Where(b => b.ExpirationDate <= current_time).Select(bid => _mapper.Map<BidDTO>(bid)).ToList();
            return new Response<BidsDTO>() { DTOObject = BidsDTO.CreateDefaultBidsPage(oldBids), IsOperationSucceeded = true, SuccessOrFailureMessage = this.getSuccessMessage() };
        }
